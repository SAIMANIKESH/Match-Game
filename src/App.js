...
class App extends Component {
  state = {
    ...
    displayTab: tabsList[0].tabId,
    score: 0,
    timer: 60,
    isGameScoreActive: false,
  }
  ...
  setTimer = value => {
    if (value) {
      setTimer = setInterval(this.tick, 1000)
    } else {
  ...
  clickAppItem = tabId => {
    this.setState({displayTab: tabId})
  }

  clickTabItem = id => {
    const {matchImage, timer} = this.state

    const randomNumb = Math.ceil(Math.random() * imagesList.length - 1)

    if (id === matchImage.id && timer !== 0) {
      this.setState(prevState => ({
        matchImage: imagesList[randomNumb],
        score: prevState.score + 1,
      }))
    } else {
      this.setState({isGameScoreActive: true})
      this.setTimer(false)
    }
  }
  ...
  render() {
    const {matchImage, displayTab, score, timer, isGameScoreActive} = this.state

    const specificCategoryImages = imagesList.filter(
      eachItem => eachItem.category === displayTab,
    )
      ...
      <div className="container">
        <ul className="top-bar">
          ...
          <div className="score-and-timer-card">
            ...
            <li className="value-display">{score}</li>
            <img
              src="https://assets.ccbp.in/frontend/react-js/match-game-timer-img.png"
              alt="timer"
              className="timer-img"
            />
            <p className="value-display">{timer} sec</p>
          </div>
        </ul>
        <div className="app-container">
          {isGameScoreActive ? (
            <>
              <GameScore score={score} clickPlayAgain={this.clickPlayAgain} />
            </>
          ) : (
            <>
              <img
              ...
              <ul className="app-items-container">
                {tabsList.map(eachItem => (
                  <AppItem
                    key={eachItem.tabId}
                    appItem={eachItem}
                    isActive={eachItem.tabId === displayTab}
                    clickAppItem={this.clickAppItem}
                  />
                ))}
              </ul>
              <ul className="tab-items-container">
                {specificCategoryImages.map(eachItem => (
                  <TabItem
                    key={eachItem.id}
                    tabItem={eachItem}
